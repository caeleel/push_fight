...
@app.route('/game/<game>/auto', methods=['POST'])
@json_response
def auto(game):
    pid, game_manager = validate_player(game)
    if game_manager is None:
...
@app.route('/game/<game>/<action>/<target>/<x>/<y>', methods=['POST'])
@json_response
def act(game, action, target, x, y):
    pid, game_manager = validate_player(game)
    if game_manager is None:
        return {'error': 'Invalid game / pid / uuid'}
    game = game_manager.game
    ...
    if action == 'move':
        result = game.move(target, x, y)
    elif action == 'place':
        result = game.place(target, x, y)
    else:
